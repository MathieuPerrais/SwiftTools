...
//
//  Credit: Basem Emara
//  Based on: https://basememara.com/creating-thread-safe-arrays-in-swift/
//
...
public class SynchronizedArray<Element> {
    private let queue = DispatchQueue(label: "\(DispatchQueue.labelPrefix).SynchronizedArray.\(UUID().uuidString)", attributes: .concurrent)
...
public extension SynchronizedArray {
    ...
    /// Returns a reversed SynchronizedArray from the underlying Array
    ///
    /// - Returns: The reversed synchronized array
    func reversed() -> SynchronizedArray {
        var reversed: SynchronizedArray?
        queue.sync { reversed = SynchronizedArray(self.array.reversed()) }
        return reversed ?? SynchronizedArray()
    }
}
